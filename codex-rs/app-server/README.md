...
- `mcpServer/startup/interrupt` — cancel startup for one configured MCP server in the specified loaded thread; returns `{}` and, if the server was still starting, emits `mcpServer/startupStatus/updated` with `status: "cancelled"`.
